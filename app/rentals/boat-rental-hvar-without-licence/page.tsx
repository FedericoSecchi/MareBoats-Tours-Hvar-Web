...
export default function NoLicencePage() {
  const rrRows = [4, 5, 8].map((n) => ({
    n,
    sharedTotal: n * rrShared,
    privateTotal: rrPrivateHalf,
    privateWins: n * rrShared > rrPrivateHalf,
  }));
  const bcRows = [5, 6, 8].map((n) => ({
    n,
    sharedTotal: n * bcShared,
    privateTotal: bcPrivate,
    privateWins: n * bcShared > bcPrivate,
  }));
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-10">
            <h3 className="font-display text-lg font-bold uppercase tracking-[-0.01em] text-[color:var(--white)]">
              Red Rocks south coast
            </h3>
            <p className="mt-1 font-body text-sm text-[color:var(--gray)]">
              Shared: &euro;{rrShared}/person &middot; Private half day: &euro;{rrPrivateHalf}{' '}
              whole boat
            </p>
              ...
              <table className="w-full font-body text-sm">
                <thead>
                  <tr className="border-b border-[color:var(--border)] bg-[color:var(--bg)]">
                    <th className="px-4 py-3 text-left font-semibold text-[color:var(--gray)]">
                      Group
                    </th>
                    <th className="px-4 py-3 text-left font-semibold text-[color:var(--gray)]">
                      Shared total
                ...
                <tbody className="divide-y divide-[color:var(--border)]">
                  {rrRows.map(({ n, sharedTotal, privateTotal, privateWins }) => (
                    <tr key={n} className={privateWins ? 'bg-[color:var(--accent)]/5' : ''}>
                      <td className="px-4 py-3 text-[color:var(--white)]">{n} people</td>
                      <td className="px-4 py-3 text-[color:var(--gray)]">
                        &euro;{fmt(sharedTotal)}
                      </td>
                      <td
          ...
          <div className="mt-8">
            <h3 className="font-display text-lg font-bold uppercase tracking-[-0.01em] text-[color:var(--white)]">
              Blue Cave and 5 Islands
            </h3>
            <p className="mt-1 font-body text-sm text-[color:var(--gray)]">
              Shared: &euro;{bcShared}/person &middot; Private: &euro;{bcPrivate} whole boat
            </p>
            <div className="mt-4 overflow-hidden rounded-xl border border-[color:var(--border)]">
              <table className="w-full font-body text-sm">
                <thead>
                ...
                <tbody className="divide-y divide-[color:var(--border)]">
                  {bcRows.map(({ n, sharedTotal, privateTotal, privateWins }) => (
                    <tr key={n} className={privateWins ? 'bg-[color:var(--accent)]/5' : ''}>
                      <td className="px-4 py-3 text-[color:var(--white)]">{n} people</td>
                      <td className="px-4 py-3 text-[color:var(--gray)]">
                        &euro;{fmt(sharedTotal)}
